...
extension TextCoordinator {
    struct SearchOptions: OptionSet {
        ...
        static let caseInsensitive = SearchOptions(rawValue: 1 << 0)
        static let wholeWords = SearchOptions(rawValue: 1 << 1)
        static let regularExpression = SearchOptions(rawValue: 1 << 2)
        static let backwards = SearchOptions(rawValue: 1 << 3)
