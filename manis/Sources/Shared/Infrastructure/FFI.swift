    ...
    public enum Error: Swift.Error, LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case let .creationFailed(message):
                "Failed to create mihomo instance: \(message)"

            case let .destroyFailed(message):
                "Failed to destroy mihomo instance: \(message)"

            case .invalidHandle:
                "Invalid mihomo instance handle"

            case .alreadyCreated:
                "Mihomo instance already created for this config"
    ...
    public class Client {
        ...
        public init(configPath: String) throws {
            let result = configPath.withCString { ptr in
                FFI.mihomo_create(configPath: ptr)
            }
        ...
        deinit {
            let result = FFI.mihomo_destroy(handle: handle)
        ...
        private static func getLastError() -> String {
            let cString = FFI.mihomo_get_last_error()
            defer { FFI.mihomo_free_string(ptr: cString) }
