...
final class TrafficDomain {
    ...
    static let shared = TrafficDomain()
        ...
        -> [TrafficCaptureMode: [TrafficCaptureDriverDescriptor]]
    {
        ...
        for key in mapping.keys {
            mapping[key]?.sort { $0.name < $1.name }
        }
...
enum SystemProxyConfigurator {
    static func updateProxyConfiguration(
        ...
        guard let prefs = SCPreferencesCreate(nil, "com.sonqyau.manis.proxy" as CFString, nil) else {
...
final class SystemConfigurationGlobalDriver: SystemProxyDriver {
    ...
    private let logger = MainLog.shared.logger(for: .proxySettings)
...
private func getExecutableURL() -> URL? {
    ...
    return Bundle.main.url(forResource: "manis", withExtension: nil, subdirectory: "Resources")
}
...
final class ProcessManualDriver: ManualProcessDriver {
    ...
    private let logger = MainLog.shared.logger(for: .proxy)
...
final class TunDeviceDriver: TunnelDriver {
    ...
    private let logger = MainLog.shared.logger(for: .tunnel)
...
final class PfTransparentProxyDriver: TunnelDriver {
    ...
    let supportedModes: Set<TrafficCaptureMode> = [.tun]
    private let logger = MainLog.shared.logger(for: .tunnel)
    private var anchorName = "manis/proxy"
